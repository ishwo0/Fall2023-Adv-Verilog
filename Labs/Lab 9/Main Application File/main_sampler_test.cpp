...
double get_duty(XadcCore *adc_p, double in_min, double in_max, double out_min, double out_max) {
...
void spectrum(PwmCore *pwm_p, XadcCore *adc_p, double duty_pot) {
   ...
   pwm_p->set_freq(50);
	if (duty_pot >= 0.0 && duty_pot <= (1.0/6.0)) {
		...
		pwm_p->set_duty(get_duty(adc_p, 0, (1.0/6.0), 0.0, 1.0), 1);
		pwm_p->set_duty(0.0, 0);
//		uart.disp("1/6\n");

	}
	else if(duty_pot > (1.0/6.0) && duty_pot <= (2.0/6.0)) {
		pwm_p->set_duty(get_duty(adc_p, (1.0/6.0), (2.0/6.0), 1.0, 0.0), 2);
...
//		uart.disp("2/6\n");
		...
		pwm_p->set_duty(0.0, 2);
		pwm_p->set_duty(1.0, 1);
		pwm_p->set_duty(get_duty(adc_p, (2.0/6.0), (3.0/6.0), 0.0, 1.0), 0);
//		uart.disp("3/6\n");
	}
   else if(duty_pot > (3.0/6.0) && duty_pot <= (4.0/6.0)) {
		pwm_p->set_duty(0.0, 2);
		pwm_p->set_duty(get_duty(adc_p, (3.0/6.0), (4.0/6.0), 1.0, 0.0), 1);
...
//		uart.disp("4/6\n");
		...
		pwm_p->set_duty(get_duty(adc_p, (4.0/6.0), (5.0/6.0), 0.0, 1.0), 2);
		pwm_p->set_duty(0.0, 1);
		pwm_p->set_duty(1.0, 0);
//		uart.disp("5/6\n");
	}
		...
		pwm_p->set_duty(1.0, 2);
		pwm_p->set_duty(0.0, 1);
		pwm_p->set_duty(get_duty(adc_p, (5.0/6.0), 1.0, 1.0, 0.0), 0);
//		uart.disp("6/6\n");
	}
...
//      double speed_old;
      double speed_new = adc_speed(&adc);
//
//
//
...
//      }
      ...
      spectrum(&pwm, &adc, speed_new);

      int digit_one = (int) (speed_new);
      int digit_ten = (int) (speed_new * 10.0) % 10;
      int digit_hun = (int) (speed_new * 100.0) % 10;
      int digit_thousand = (int) (speed_new * 1000.0) % 10;

      sseg.write_1ptn((sseg.h2s(digit_one)), 3);
      sseg.write_1ptn((sseg.h2s(digit_ten)), 2);
      sseg.write_1ptn((sseg.h2s(digit_hun)), 1);
      sseg.write_1ptn((sseg.h2s(digit_thousand)), 0);
      // decimal point follows binary pattern: 0000_0100.
      sseg.set_dp(0x08);
